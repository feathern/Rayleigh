Module Linear_Solve_QI
  ...
  Type Equation
     ...
     real*8, Allocatable :: lhs_coefs(:,:,:)

     ! %dia is the sparse matrix in diagonal format: %dia%dat(:,:) are the coefs, %dia%nrow, ncol, nl, nu
     ! %lu(:,:) is the LU factorization
     ! %piv(:) are the pivots
     Type(ddia_and_lu) :: QI_MMDTL, QI_MPDTL
...
Contains
  ...
  Subroutine Use_QISolve()
    qi_flag = .true.
  End Subroutine Use_QISolve
  ...
  Subroutine Reset_Equation_Coefficients()
    ...
    Integer :: i, j, k
    Do k = 1, n_equations
       If (qi_flag.and.(k.eq.zeq)) Then !Added by Kyle A.
          Do j=1,n_modes
             If(Allocated(equation_set(j,k)%QI_MMDTL%dia%dat )) equation_set(j,k)%QI_MMDTL%dia%dat(:,:)=0d0
             If(Allocated(equation_set(j,k)%QI_MPDTL%%dia%dat)) equation_set(j,k)%QI_MPDTL%dia%dat(:,:)=0d0
             If(Allocated(equation_set(j,k)%QI_MMDTL%lu)) equation_set(j,k)%QI_MMDTL%lu(:,:)=0d0
             If(Allocated(equation_set(j,k)%QI_MPDTL%lu)) equation_set(j,k)%QI_MPDTL%lu(:,:)=0d0
             If(Allocated(equation_set(j,k)%QI_MMDTL%piv)) equation_set(j,k)%QI_MMDTL%piv(:)=0d0
             If(Allocated(equation_set(j,k)%QI_MPDTL%piv)) equation_set(j,k)%QI_MPDTL%piv(:)=0d0
       ...
       If (equation_set(j,k)%solvefor) Then
          If (qi_flag.and.(k.eq.zeq)) Then !Added by Kyle A.
             If (equation_set(j,k)%primary) Then
                If(Allocated(equation_set(j,k)%QI_MMDTL%dia%dat)) Deallocate(equation_set(j,k)%QI_MMDTL%dia%dat)
                If(Allocated(equation_set(j,k)%QI_MPDTL%%dia%dat)) Deallocate(equation_set(j,k)%QI_MPDTL%dia%dat)
                
                If(Allocated(equation_set(j,k)%QI_MMDTL%lu)) Deallocate(equation_set(j,k)%QI_MMDTL%lu)
                If(Allocated(equation_set(j,k)%QI_MPDTL%lu)) Deallocate(equation_set(j,k)%QI_MPDTL%lu)
                
                If(Allocated(equation_set(j,k)%QI_MMDTL%piv)) Deallocate(equation_set(j,k)%QI_MMDTL%piv)
                If(Allocated(equation_set(j,k)%QI_MPDTL%piv)) Deallocate(equation_set(j,k)%QI_MPDTL%piv)
       ...
       If (qi_flag.and.(k.eq.zeq)) Then
          Do j=1,n_modes
             If (equation_set(j,k)%primary .and. equation_set(j,k)%solvefor) Then
                Call QI_Solve(A, b, nrhs, ldb, verb) !Added by Kyle A., this need to be properly implemented.
             EndIf
          EndDo
       Else
  ...
  Subroutine LU_Decompose_Matrices()
    ...
    Do k = 1, n_equations
       If (qi_flag.and.(k.eq.zeq)) Then
          If (equation_set(j,k)%primary .and. equation_set(j,k)%solvefor) Then
             Call QI_Decompose() !Added by Kyle A., If necessary call do LU decomposition for QI matrices.
          EndIf
       Else
          Do j = 1, n_modes
             If (equation_set(j,k)%primary .and. equation_set(j,k)%solvefor) Then
  ...
  Subroutine Add_Implicit_Term_QI(eqind,varind,dorder,amp,mode,static)
    ...
    Endif
  End Subroutine Add_Implicit_Term_QI
  ...
  Subroutine Set_All_RHS(buffer)
    ...
    ! If the buffers are linked, take into account interleaving.
    Implicit None
    Real*8, Intent(InOut) :: buffer(:,:,:,1:)
    Integer :: i, ind,istart,iend 
    ...
    Do i = 1, n_equations
       ...
       !The first index here is radius, phase, and then the ell index and associated mode size.
  ...
  Subroutine Load_Interior_Rows_Cheby(row,col,amp,dorder,mpointer)
    Implicit None
    Integer, Intent(In) :: row, col, dorder !row and col are zero, one, or two, dorder = Derivative order
    Integer :: r, n, off1,npoly, hh, i, nsub
    real*8, Intent(In) :: amp(:) !Function of radius 1:nr
  ...
  Subroutine Prep_RHS_QI(eqind)
    ...
    Do ivar = 1, howmany_vars
       ...
       equation_set(1,eqind)%rhs_cheby(ista+1:ista+dim1-orders(ivar),:,:) = buf2
       DeAllocate(buf2)
    End Do
