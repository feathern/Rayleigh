...
A simulation's behavior, including run initiation, output etc. can be controlled through the main\_input file.  In addition subset of the options available through main\_input may also be specified at the command line.  Any value specified at the command line takes precedence over that specified in the main\_input file.   Several examples of command line parameters are provided in throughout this manual.  Available command line parameters are described in full in Section \ref{sec:cmd}.
