...
Contains
    ...
    Subroutine Write_Checkpoint(abterms,iteration,dt,new_dt,elapsed_time, input_file)
        ...
        Else
            ...
            If (my_rank .eq. 0) Then
                Call Make_Directory(Trim(my_path)//checkpoint_prefix,ecode)
                input_file = Trim(my_path)//trim(checkpoint_prefix)//'/'//'input'
            Endif
    ...
    Subroutine Read_Checkpoint(fields, abterms,iteration,read_pars)
        ...
        If (my_rank .eq. 0) Then
            ...
            cfile = Trim(checkpoint_prefix)//'/'//'grid_etc'
    ...
    Subroutine Read_Checkpoint_Legacy(fields, abterms,iteration,read_pars)
        Implicit None
        Integer, Intent(In) :: iteration, read_pars(1:2)
        Real*8, Intent(InOut) :: fields(:,:,:,:), abterms(:,:,:,:)
        Integer :: n_r_old, l_max_old, grid_type_old, nr_read
        ...
        If (my_row_rank .eq. 0) Then
            !If (nr_read .gt. 0) Then    ! SMALL BUG HERE RELATED TO MPI_IO Logic... -- Revist this
                ...
                If (read_hydro .eq. 1) Then
                    Call Read_Field_Legacy(rowstrip,1,wchar, iteration,nr_read,nlm_total_old)
                    Call Read_Field_Legacy(rowstrip,2,pchar, iteration,nr_read,nlm_total_old)
                    Call Read_Field_Legacy(rowstrip,3,tchar, iteration,nr_read,nlm_total_old)
                    Call Read_Field_Legacy(rowstrip,4,zchar, iteration,nr_read,nlm_total_old)
                Endif
                offset_index = 4
                If (magnetism) Then
                    If (read_magnetism .eq. 1) Then
                        Call Read_Field_Legacy(rowstrip,5,cchar, iteration,nr_read,nlm_total_old)
                        Call Read_Field_Legacy(rowstrip,6,achar, iteration,nr_read,nlm_total_old)
                    Endif
                    offset_index = 6
                Endif

                If (read_hydro .eq. 1) Then
                    Call Read_Field_Legacy(rowstrip,offset_index+1,'WAB', iteration,nr_read,nlm_total_old)
                    Call Read_Field_Legacy(rowstrip,offset_index+2,'PAB', iteration,nr_read,nlm_total_old)
                    Call Read_Field_Legacy(rowstrip,offset_index+3,'TAB', iteration,nr_read,nlm_total_old)
                    Call Read_Field_Legacy(rowstrip,offset_index+4,'ZAB', iteration,nr_read,nlm_total_old)
                Endif
                If (magnetism) Then
                    If (read_magnetism .eq. 1) Then
                        Call Read_Field_Legacy(rowstrip,offset_index+5,'CAB', iteration,nr_read,nlm_total_old)
                        Call Read_Field_Legacy(rowstrip,offset_index+6,'AAB', iteration,nr_read,nlm_total_old)
                    Endif
                Endif
    ...
    End Subroutine Read_Checkpoint_Legacy
    ...
    Subroutine Write_Field(arr,ind,tag,iter)
                ...
                cfile = Trim(my_path)//Trim(checkpoint_prefix)//'/'//Trim(tag)
    ...
    Subroutine Read_Field(arr,ind,tag,iter,nread,nlm)
        ...
        cfile = Trim(checkpoint_prefix)//'/'//trim(tag)
    ...
    Subroutine Read_Field_Legacy(arr,ind,tag,iter,nread,nlm)
        Implicit None
        Integer, Intent(In) :: ind, iter,nread, nlm
        ...
        if (ierr .ne. 0) Then
                Write(6,*)'Error closing file: ', pfi%ccomm%rank
        Endif
    End Subroutine Read_Field_Legacy
